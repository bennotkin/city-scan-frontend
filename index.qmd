...
input_dir <- paste0("mnt/input/")

# City parameters
city_params <- read_yaml(paste0(input_dir, "city_inputs.yml"))
# cities <- list.files("cities")
city <- city_params$city_name
city_string <- tolower(city) %>% stringr::str_replace_all(" ", "-")
output_dir <- "styled-maps/"
spatial_dir <- paste0(input_dir, "spatial/")
...
```{r}
# Get text
city_text <- read_md("mnt/input/text-files/manual-text.md")
...
# Define the AOI
aoi <- st_read(paste0(input_dir, "AOI"))
